...
<%!ResultSet rs, rq, data, data2, data3, data4, data5, re, selectData, rs2,comp;
	ResultSetMetaData rsmd;
	Object ob;
	DataService sr1, ds1, ds2;
	int x;
											...
											<!-- <div class="form-group">
												...
												<div class="col-sm-4">
													...
													<%
														...
														System.out.print("productId>>" + pid);
													%>

													<input autocomplete="off" name="productId" required type="text"
														class="form-control" id="productId" value="<%=pid%>" readonly="readonly">
										...
										<table id="example"
											...
											<thead>
												<tr class="headings fix_back_color">
													...
													<th>Version</th>
													<th>Date</th>
													<th>Batch Size(Kg's)</th>					
													<th>Batch Size(No's)</th>		
													
												</tr>
											</thead>
											<tbody>
												<tr>
												<!-- //New updatein code  dt:5/10/23 -->
												<!-- yaha se -->
												
												<%ds2=new DataService();
												String Ccode = data.getString("companyId");
												comp=ds2.Company_list_ById(Ccode);
												while (comp.next())
												{
												
												%>
													<td id="com" value=<%=data.getString("companyId")%>><%=comp.getString("Cname")%>(<%=comp.getString("code")%>)</td>
													<%} %>
													<!-- yaha Tk -->
													
													<td id="productId" ><%=data.getString("productCode")%></td>
													<td id="productName" ><%=data.getString("productName")%></td>
													<td id="version" ><%=data.getString("version")%></td>
													<td id="date" ><%=data.getString("date")%></td>
													<td id="batchKg" ><%=data.getString("batchSizeKg")%></td>
													<td id="batchNo" ><%=data.getString("batchSizeNo")%></td>
									...
									<div class="row">
										<div class="col-md-12">
											<div class="form-group">
												...
												<button onclick="printPage()" value=""
										class="info btn btn-info btn_info">Print</button>
													<!-- <button type="submit" value="update"
														class="btn btn-success update "  id="details">Print</button> -->
	...
	var pid = document.getElementById("productId").value;
	   ...
	   $.ajax({
           ...
           success : function(response) {
				...
				$.each(response.dataprod, function(Index,prod){
					//alert(prod.companyId);
					//$("#com").append(("<option></option>").attr("value", prod.com).text(prod.com));
					$("#com").val(prod.com);
					$("#version2").val(prod.version2);
					$("#date").val(prod.date);
					$("#batchKg").val(prod.batchKg);
					$("#batchNo").val(prod.batchNo);
					$("#productName").val(prod.productName);
					//$("#item").append('<option value="'+prod.iName+'">'+prod.iname+'</option>');
					/* alert(prod.com);
					alert(prod.version2);
					alert(prod.date);
					alert(prod.batchkg);
					...
					console.log(prod.version2)
					console.log(prod.date);
					console.log(prod.batchKg);
					console.log(prod.batchNo);
					console.log(prod.iName); */

              	  tableData = tableData + '<tr>';
              	 // tableData = tableData +   '<td><a class="cut" onclick="cuting(this);">-</a><span ></span></td> ';
              	  tableData = tableData +   '<td id="iName">'+prod.iName+'</td> ';
              	  tableData = tableData +   '<td id="unit">'+prod.unit+'</td> ';
              	  tableData = tableData +   '<td idd="itemQty">'+prod.qty+'</td> ';
              	  tableData = tableData +  '</tr>';
