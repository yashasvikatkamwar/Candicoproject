...
@WebServlet("/Company_next_PO_ID_get_COMBO")
public class Company_next_PO_ID_get_COMBO extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("Company_next_PO_ID_get_COMBO=combobox===>");
...
//      System.out.println("compname=combobox===>"+compname);
			...
			rs = sr.Company_PO_ID_next(Cid.trim());
			System.out.println("company_name=combobox===>123" + Cid);

			while (rs.next()) {
				// rs2= s.executeQuery("select
				// Unit_id,emp_id,First_name,Last_name,designation,Ifsc_no,ESIC_no,pf_no_a,bank_name,accno
				// from security_recruityment where Unit_name='"+pr+"'");
...
//           String h1=rs.getString(2).replace(" ","_");
				json.addProperty("text", rs.getString(1));
				json.addProperty("sr", rs.getString(1));
				data_json.add(json);
